...
float3 CalculateCustomLighting(CustomLightingData d) {
	...
	for (uint lightI = 0; lightI < numAdditionalLights; lightI++) {
		...
		Light light = GetAdditionalLightForToon(lightI, d.positionWS);
		half NdotL = saturate(dot(d.normalWS, light.direction));
		half atten = light.distanceAttenuation * light.shadowAttenuation;// *GetLightIntensity(light.color);
		half thisDiffuse = atten * NdotL;
		half thisSpecular = LightingSpecular(thisDiffuse, light.direction, d.normalWS, d.viewDirectionWS, 1, d.smoothness);
		//Diffuse += thisDiffuse;
...
void CalculateCustomLighting_float(float3 Position, float3 Normal, float3 ViewDirection,
	...
	float2 LightmapUV,
	out float3 Direction, out float3 Color, out half DistanceAtten, out half ShadowAtten) {
